...
public class Square extends Rectangle implements Snappable {
    ...
    public Quadrilateral snap() {
    	...
    	TwoDPoint[] vertexes = this.getVertices();
    	for(int i = 0; i < vertexes.length; i++)
    	{
    		TwoDPoint point = new TwoDPoint(Math.round(vertexes[i].getx()), Math.round(vertexes[i].gety()));
    		vertices.add(point);
    	}
    	Quadrilateral ret = new Quadrilateral(vertices);
    	return ret;
    }
}